...
class CadastroAjaxRequestViewsTestCase(BaseTestCase):
    ...
    def test_consulta_pessoa_get_request_filtra_empresa_ativa(self):
        cliente = Cliente.objects.create(
            nome_razao_social='Cliente Consulta Lookup',
            tipo_pessoa='PJ',
            empresa_relacionada=self.empresa_ativa,
        )
        Cliente.objects.create(
            nome_razao_social='Cliente Outra Empresa Lookup',
            tipo_pessoa='PJ',
            empresa_relacionada=Empresa.objects.create(
                nome_razao_social='Empresa Lookup Externa',
                tipo_pessoa='PJ',
            ),
        )

        response = self.client.get(reverse('cadastro:consultapessoa'), {
            'tipo': 'cliente',
            'term': 'Cliente Consulta',
        })
        self.assertEqual(response.status_code, 200)
