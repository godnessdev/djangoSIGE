...
class LancamentoListBaseView(CustomListView, MovimentoCaixaMixin):
    ...
    def get_queryset(self, object, status):
        related_fields = ['empresa', 'grupo_plano']
        if object is Saida:
            related_fields.append('fornecedor')
        elif object is Entrada:
            related_fields.append('cliente')
        queryset = object.objects.filter(status__in=status).select_related(
            *related_fields).order_by('-id')
...
class LancamentoListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        all_entradas = self.filtrar_queryset(
            Entrada.objects.select_related('empresa', 'grupo_plano', 'cliente').all())
        all_saidas = self.filtrar_queryset(
            Saida.objects.select_related('empresa', 'grupo_plano', 'fornecedor').all())
        all_lancamentos = sorted(
            chain(all_saidas, all_entradas),
...
class ContaPagarAtrasadasListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        queryset = Saida.objects.filter(
            data_vencimento__lt=datetime.now().date(), status__in=['1', '2']).select_related(
            'empresa', 'grupo_plano', 'fornecedor').order_by('-id')
        return self.filtrar_queryset(queryset)
...
class ContaPagarHojeListView(ContaPagarAtrasadasListView):
    ...
    def get_queryset(self):
        queryset = Saida.objects.filter(
            data_vencimento=datetime.now().date(), status__in=['1', '2']).select_related(
            'empresa', 'grupo_plano', 'fornecedor').order_by('-id')
        return self.filtrar_queryset(queryset)
...
class ContaReceberAtrasadasListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        queryset = Entrada.objects.filter(
            data_vencimento__lt=datetime.now().date(), status__in=['1', '2']).select_related(
            'empresa', 'grupo_plano', 'cliente').order_by('-id')
        return self.filtrar_queryset(queryset)
