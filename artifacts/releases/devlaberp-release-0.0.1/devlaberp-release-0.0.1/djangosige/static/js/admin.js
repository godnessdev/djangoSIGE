...
$.Admin.pessoaForm = {
    ...
    initCnpjLookup: function(){
        ...
        if(_this._cnpjLookupInitialized){
            return;
        }
        _this._cnpjLookupInitialized = true;
        ...
        $('body').on('input', 'input[data-cnpj-lookup-field="cnpj"][data-cnpj-autolookup="true"]', function(){
            var input = $(this);
            var form = input.closest('form');
            var button = input.closest('.cnpj-lookup-group').find('.js-cnpj-lookup');
            var digits = (input.val() || '').replace(/\D/g, '');
            var timerKey = form.attr('id') || 'default';

            if(_this._cnpjLookupTimers[timerKey]){
                clearTimeout(_this._cnpjLookupTimers[timerKey]);
            }

            if(digits.length < 14){
                _this.setCnpjLookupStatus(form, 'idle', 'Digite o CNPJ para buscar os dados automaticamente.');
                return;
            }

            _this._cnpjLookupTimers[timerKey] = setTimeout(function(){
                if(button.length && button.data('lastLookupCnpj') !== digits){
    ...
    lookupCnpj: function(button){
        ...
        if(cnpj.length !== 14){
            _this.setCnpjLookupStatus(form, 'error', 'Informe um CNPJ valido com 14 digitos.');
            if(window.AppCore && window.AppCore.messages){
        ...
        _this.setCnpjLookupStatus(form, 'loading', 'Consultando CNPJ...');
        button.prop('disabled', true).addClass('is-loading');
        button.data('lastLookupCnpj', cnpj);
        ...
        $.ajax({
            ...
            success: function(response){
                if(!response || !response.success || !response.data){
                    _this.setCnpjLookupStatus(form, 'error', (response && response.error) || 'Nao foi possivel consultar o CNPJ informado.');
                    if(window.AppCore && window.AppCore.messages){
                ...
                _this.setCnpjLookupStatus(form, 'success', 'Empresa encontrada. Dados preenchidos automaticamente.');
                if(window.AppCore && window.AppCore.messages){
            ...
            error: function(xhr){
                ...
                _this.setCnpjLookupStatus(form, 'error', response.error || 'Nao foi possivel consultar o CNPJ informado.');
                if(window.AppCore && window.AppCore.messages){
    ...
    applyCnpjData: function(form, data){
        ...
        this.setFieldValue(form, 'input[name$="inscricao_estadual"]', data.inscricao_estadual);
    ...
    setCnpjLookupStatus: function(form, tone, text){
        var status = form.find('.js-cnpj-lookup-status').first();
        if(!status.length){
            return;
        }
        status.attr('data-tone', tone || 'idle');
        status.text(text || '');
    },
...
$.Admin.remoteLookup = {
    init: function(){
        var _this = this;
        _this.initSelects(document);
        $(document).off('formCreated.remoteLookup').on('formCreated.remoteLookup', '.formset', function(){
            _this.initSelects(this);
        });
    },

    initSelects: function(root){
        var _this = this;
        if (typeof $.fn.autocomplete !== 'function') {
            return;
        }

        $(root).find('select.js-remote-select[data-lookup-url]').each(function(){
            _this.enhanceSelect($(this));
        });
    },

    enhanceSelect: function($select){
        var _this = this;
        var lookupUrl = $select.attr('data-lookup-url');
        var placeholder = $select.attr('data-lookup-placeholder') || 'Buscar...';
        var selectedText = $select.find('option:selected').text();
        var selectedValue = $select.val();
        var $wrapper;
        var $input;

        if (!lookupUrl || $select.data('remoteLookupReady')) {
            return;
        }

        $wrapper = $('<div class="remote-lookup-shell"></div>');
        $input = $('<input type="search" class="form-control remote-lookup-input" autocomplete="off">');
        $input.attr('placeholder', placeholder);

        if (selectedValue && selectedText && selectedText !== '----------') {
            $input.val(selectedText);
            $select.attr('data-lookup-selected-label', selectedText);
        }

        $select.before($wrapper);
        $wrapper.append($input);
        $select.addClass('hidden remote-lookup-original');
        $select.data('remoteLookupReady', true);
        ...
        $input.autocomplete({
                    ...
                    success: function(payload){
                        ...
                        response($.map(results, function(item){
                            return {
                                ...
                                value: item.label || item.descricao || '',
                                id: item.id,
                            };
                        }));
                    },
            ...
            select: function(event, ui){
                event.preventDefault();
                _this.setSelectedValue($select, ui.item.id, ui.item.label);
                $input.val(ui.item.label);
            },
            change: function(event, ui){
                ...
                if (!$input.val()) {
                    _this.clearSelectedValue($select);
                    return;
                }

                if ($input.val() !== ($select.attr('data-lookup-selected-label') || '')) {
                    _this.clearSelectedValue($select);
                    $input.val('');
                }
            }
    ...
    setSelectedValue: function($select, value, label){
        var $blankOption = $select.find('option[value=""]').first();
        $select.find('option').not($blankOption).remove();
        $select.append(new Option(label, value, true, true));
        $select.attr('data-lookup-selected-label', label);
        $select.val(value).trigger('change');
    },

    clearSelectedValue: function($select){
        var $blankOption = $select.find('option[value=""]').first();
        $select.find('option').not($blankOption).remove();
        $select.attr('data-lookup-selected-label', '');
...
$.Admin.auth = {
    init: function(){
        if (typeof $.fn.mask === 'function') {
            $('input[data-cnpj-lookup-field="cnpj"]').mask('99.999.999/9999-99', {reverse: true});
        }

        $('body').off('submit.authSubmit').on('submit.authSubmit', '.js-auth-submit-form', function(){
            var form = $(this);
            var button = form.find('.js-auth-submit-button');
            if(!button.length || button.prop('disabled')){
                return;
            }

            button.data('original-text', button.text());
            button.text(button.data('loadingText') || 'Processando...');
            button.prop('disabled', true).addClass('is-loading');
        });
    }
}
...
$(function () {
    ...
    $.Admin.auth.init();
    $.Admin.pessoaForm.initCnpjLookup();
    $.Admin.remoteLookup.init();
});
