...
from .forms import UserLoginForm, UserRegistrationForm, PasswordResetForm, SetPasswordForm, PerfilUsuarioForm, InitialSetupEmpresaForm, InitialSetupAdminForm
...
from djangosige.apps.cadastro.views.ajax_views import ConsultaCNPJ as CadastroConsultaCNPJ
...
def initial_setup_required():
    return not Empresa.objects.exists() or not User.objects.filter(is_superuser=True).exists()


def get_initial_setup_company(request):
    empresa_id = request.session.get('initial_setup_empresa_id')
    if empresa_id:
        empresa = Empresa.objects.filter(pk=empresa_id).first()
        if empresa:
            return empresa
        request.session.pop('initial_setup_empresa_id', None)
...
class UserFormView(View):
    ...
    initial_setup_template_name = 'login/instalacao_inicial.html'

    def _get_initial_setup_context(self, request, form):
        empresa = get_initial_setup_company(request)
        install_step = 'empresa' if empresa is None else 'admin'
        return {
            'form': form,
            'install_step': install_step,
            'empresa_instalacao': empresa,
            'cnpj_lookup_url': reverse_lazy('login:consultacnpjview'),
        }

    def _render_initial_setup(self, request, form):
        return render(
            request,
            self.initial_setup_template_name,
            self._get_initial_setup_context(request, form),
        )

    def _handle_initial_setup(self, request):
        empresa = get_initial_setup_company(request)
        if empresa is None:
            form = InitialSetupEmpresaForm(request.POST or None)
            if request.method == 'POST' and form.is_valid():
                empresa = form.save()
                request.session['initial_setup_empresa_id'] = empresa.pk
                messages.success(
                    request,
                    'Empresa inicial criada. Agora crie o usuario administrador.',
                )
                return redirect('login:loginview')
            return self._render_initial_setup(request, form)

        form = InitialSetupAdminForm(request.POST or None)
        if request.method == 'POST' and form.is_valid():
            user = form.save(empresa)
            request.session.pop('initial_setup_empresa_id', None)
    ...
    def get(self, request):
        if initial_setup_required():
            return self._handle_initial_setup(request)
...
class InitialSetupConsultaCNPJView(CadastroConsultaCNPJ):
    pass
