...
def get_nota_saida_queryset(user):
    empresa = get_empresa_ativa(user)
    if empresa is None:
        return NotaFiscalSaida.objects.none()
    return NotaFiscalSaida.objects.filter(emit_saida=empresa)


def get_nota_entrada_queryset(user):
    empresa = get_empresa_ativa(user)
    if empresa is None:
        return NotaFiscalEntrada.objects.none()
    return NotaFiscalEntrada.objects.filter(dest_entrada=empresa)


def get_configuracao_nfe_empresa(empresa):
    if empresa is None:
        return None
    configuracao, _ = ConfiguracaoNotaFiscal.objects.get_or_create(
        empresa=empresa)
    return configuracao


def get_proximo_numero_nfe_saida(empresa):
    if empresa is None:
        return 1

    numeros = []
    for numero in NotaFiscalSaida.objects.filter(
            emit_saida=empresa).values_list('n_nf_saida', flat=True):
        try:
        ...
        if (form.is_valid() and aut_form.is_valid()):
            ...
            if isinstance(self.object, NotaFiscalSaida):
                if empresa is not None:
                    self.object.emit_saida = empresa
                self.atualizar_campos(request.POST)
            elif isinstance(self.object, NotaFiscalEntrada) and empresa is not None:
                self.object.dest_entrada = empresa
            self.object.save()
...
class AdicionarNotaFiscalSaidaView(AdicionarNotaFiscalView):
    ...
    def set_form_initial_data(self, form, user):
        ...
        empresa = get_empresa_ativa(user)
        if empresa is not None:
            form.initial['emit_saida'] = empresa

        conf_nfe = get_configuracao_nfe_empresa(empresa)
        if conf_nfe is not None:
        ...
        form.initial['n_nf_saida'] = get_proximo_numero_nfe_saida(empresa)
...
class NotaFiscalSaidaListView(NotaFiscalListView):
    ...
    def get_queryset(self):
        return get_nota_saida_queryset(self.request.user)
...
class NotaFiscalEntradaListView(NotaFiscalListView):
    ...
    def get_queryset(self):
        return get_nota_entrada_queryset(self.request.user)
...
class GerarNotaFiscalSaidaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pedido = get_object_or_404(
            PedidoVenda.objects.filter(empresa=get_empresa_ativa(request.user)),
            id=pedido_id)
        ...
        conf_nfe = get_configuracao_nfe_empresa(pedido.empresa)
        if conf_nfe is not None:
        ...
        nova_nota.n_nf_saida = get_proximo_numero_nfe_saida(pedido.empresa)
        nova_nota.emit_saida = pedido.empresa
...
class ConfiguracaoNotaFiscalView(FormValidationMessageMixin, CustomTemplateView):
    ...
    def get_object(self):
        return get_configuracao_nfe_empresa(get_empresa_ativa(self.request.user))
...
class ValidarNotaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        nota_obj = get_object_or_404(get_nota_saida_queryset(request.user), id=nfe_id)
...
class EmitirNotaView(CustomTemplateView):
    ...
    def get_object(self, pk):
        return get_object_or_404(get_nota_saida_queryset(self.request.user), pk=pk)
...
class GerarCopiaNotaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        instance = get_object_or_404(get_nota_saida_queryset(request.user), id=nota_id)
        ...
        instance.n_nf_saida = get_proximo_numero_nfe_saida(instance.emit_saida)
        instance.n_fat = None
        instance.save()
...
class GerarDanfeView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        nota_id = kwargs.get('pk', None)
        nota_obj = get_object_or_404(get_nota_saida_queryset(request.user), pk=nota_id)
...
class ConsultarCadastroView(CustomTemplateView):
    ...
    def consultar_cadastro(self, empresa, salvar_arquivos):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.consultar_cadastro(
            empresa, salvar_arquivos, empresa_emitente=empresa)
    ...
    def get(self, request, *args, **kwargs):
        form = ConsultarCadastroForm(user=request.user)
        return self.render_to_response(self.get_context_data(form=form,))

    def post(self, request, *args, **kwargs):
        form = ConsultarCadastroForm(request.POST, user=request.user)
...
class InutilizarNotasView(CustomTemplateView):
    ...
    def inutilizar_notas(self, empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos):
        ...
        processador_nota.inutilizar_notas(
            empresa, ambiente, modelo, serie, numero_inicial, numero_final,
            justificativa, salvar_arquivos, empresa_emitente=empresa)
    ...
    def get(self, request, *args, **kwargs):
        form = InutilizarNotasForm(user=request.user)
        return self.render_to_response(self.get_context_data(form=form,))

    def post(self, request, *args, **kwargs):
        form = InutilizarNotasForm(request.POST, user=request.user)
...
class ConsultarNotaView(CustomTemplateView):
    ...
    def consultar_nota(self, chave, ambiente, salvar_arquivos, empresa=None):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.consultar_nota(
            chave, ambiente, salvar_arquivos, empresa=empresa)
    ...
    def get(self, request, *args, **kwargs):
        form = ConsultarNotaForm(user=request.user)
        form.initial['nota'] = kwargs.get('pk', None)

        return self.render_to_response(self.get_context_data(form=form,))

    def post(self, request, *args, **kwargs):
        form = ConsultarNotaForm(request.POST, user=request.user)
        ...
        if form.is_valid():
            ...
            empresa = nota.emit_saida if nota else get_empresa_ativa(request.user)
            processo = self.consultar_nota(
                chave, ambiente, salvar_arquivos, empresa=empresa)
            return self.form_valid(form, processo)
...
class BaixarNotaView(CustomTemplateView):
    ...
    def baixar_nota(self, chave, ambiente, ambiente_nacional, salvar_arquivos, empresa=None):
        ...
        processador_nota.baixar_nota(
            chave, ambiente, ambiente_nacional, salvar_arquivos, empresa=empresa)
    ...
    def get(self, request, *args, **kwargs):
        form = BaixarNotaForm(user=request.user)
        form.initial['nota'] = kwargs.get('pk', None)

        return self.render_to_response(self.get_context_data(form=form,))

    def post(self, request, *args, **kwargs):
        form = BaixarNotaForm(request.POST, user=request.user)
...
class ManifestacaoDestinatarioView(CustomTemplateView):
    ...
    def efetuar_manifesto(self, chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos, empresa=None):
        ...
        processador_nota.efetuar_manifesto(
            chave, cnpj, ambiente, tipo_manifesto, justificativa,
            ambiente_nacional, salvar_arquivos, empresa=empresa)
    ...
    def get(self, request, *args, **kwargs):
        form = ManifestacaoDestinatarioForm(user=request.user)
        return self.render_to_response(self.get_context_data(form=form,))

    def post(self, request, *args, **kwargs):
        form = ManifestacaoDestinatarioForm(request.POST, user=request.user)
